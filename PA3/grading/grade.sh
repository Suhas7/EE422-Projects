...
perform_test_on_this_submission()
{
    ...
    if [ "$TA_MODE" = true ]
    then
	    MAVEN_OPTS="-Xss1024m -Xms1024m -Xmx2048m" mvn clean test > test_log.txt 2>&1 || true
    else
    	MAVEN_OPTS="-Xss1024m -Xms1024m -Xmx2048m" mvn clean test 2>&1 | tee test_log.txt || true
    fi
